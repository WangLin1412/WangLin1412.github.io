...
const characters = "HAPPYBIRTHDAYCHUCMUNGSINHNHATVUMINDUC";
const fontSize = 20;
...
function drawMatrixRain() {
  ...
  if (state === "forming") {
    ...
    if (now - explosionStartTime > EXPLOSION_TIME) {
      ...
      currentDisplayTime = BASE_DISPLAY_TIME + (nextText.length > 5 ? 5000 : 0);
      initDots(nextText);
      lastChangeTime = now;
      lastCharTime = now;
      state = "forming";
...
currentDisplayTime = BASE_DISPLAY_TIME + (texts[0].length > 5 ? 5000 : 0);
initDots(texts[currentTextIndex]);
